...
// job
func (re *RunEngine) StartOneRun(job *model.Job, ctx context.Context) (model.TriggerResponse, error) {
	re.runningCount.RLock()
	select {
	case <-re.conf.Close:
		re.runningCount.RUnlock()
		return model.TriggerResponse{}, errors.New("run >> the application is shutting down. Operation impossible.")
	default:
		...
		res := model.TriggerResponse{ContainerName: params.ContainerName()} // todo set run id !
